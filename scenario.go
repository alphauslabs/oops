	...
	for _, f := range in.ScenarioFiles {
		...
		if in.ReportPubsub != "" && in.app != nil {
			if in.app.rpub != nil {
				...
				if in.Metadata != nil {
					for _, key := range []string{"pr_number", "branch", "commit_sha", "actor", "trigger_type", "run_url", "repository", "workflow", "total_scenarios", "missing_tests_in_pr", "should_run_tests"} {
